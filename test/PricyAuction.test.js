    ...
    describe('placeBid()', async () => {
        ...
        describe('validation', () => {
            ...
            it('will revert if sender is smart contract', async () => {
                ...
                await expectRevert(
                    (this.biddingContract.auctionContract).placeBid(this.nft.address, TOKEN_ONE_ID, ether('0.2'), {
                        from: bidder,
                        //value: ether('0.2')
                    }),
                    "PricyAuction.placeBid: No contracts permitted"
                );
            });
            ...
            it('will fail with valid token but no auction', async () => {
                await expectRevert(
                    this.auction.placeBid(this.nft.address, TOKEN_TWO_ID, {
                        from: bidder,
                        value: ether('1')
                    }),
                    'PricyAuction.placeBid: Bidding outside of the auction window'
                );
            });

            it('will fail when auction finished', async () => {
                //await this.auction.setNowOverride('11');
                await this.auction.setTime('11');
                await expectRevert(
                    this.auction.placeBid(this.nft.address, TOKEN_ONE_ID, {
                        from: bidder,
                        value: ether('1')
                    }),
                    'PricyAuction.placeBid: Bidding outside of the auction window'
            ...
            it('will fail when outbidding someone by less than the increment', async () => {
                //await this.auction.setNowOverride('2');
                await this.auction.setTime('2');
                await this.auction.placeBid(this.nft.address, TOKEN_ONE_ID, {
                    from: bidder,
                    value: ether('0.2')
                });
                ...
                await expectRevert(
                    ...
                    'PricyAuction.placeBid: Failed to outbid highest bidder'
                );
            });
            */
        });
        ...
        describe('successfully places bid', () => {
            ...
            it('successfully increases bid', async () => {
                ...
                const bidderTracker = await balance.tracker(bidder);
                const receipt = await this.auction.placeBid(this.nft.address, TOKEN_ONE_ID, {
                    from: bidder,
                    value: ether('0.2')
                });
                ...
                const receipt2 = await this.auction.placeBid(this.nft.address, TOKEN_ONE_ID, {
                    from: bidder,
                    value: ether('1')
                });
    ...
    describe('withdrawBid()', async () => {
        ...
        it('fails with withdrawing a bid which does not exist', async () => {
            await expectRevert(
                ...
                'PricyAuction.withdrawBid: You are not the highest bidder'
            );
        });
        ...
        it('fails with withdrawing when lockout time not passed', async () => {
            ...
            await expectRevert(
                ...
                "PricyAuction.withdrawBid: Cannot withdraw until lock time has passed"
            );
        });
        ...
        it('fails when withdrawing after auction end', async () => {
            ...
            await expectRevert(
                ...
                "PricyAuction.withdrawBid: Past auction end"
            );
        });
        ...
        describe('validation', () => {
            ...
            it('cannot result if not an owner', async () => {
                await expectRevert(
                    ...
                    'PricyAuction.resultAuction: Sender must be item owner'
                );
            });
            ...
            it('cannot result if auction has not ended', async () => {
                await expectRevert(
                    ...
                    'PricyAuction.resultAuction: The auction has not ended'
                );
            });
            ...
            it('cannot result if the auction is reserve not reached', async () => {
                ...
                await expectRevert(
                    ...
                    'PricyAuction.resultAuction: reserve not reached'
                );
            });
            ...
            it('cannot result if the auction has no winner', async () => {
                ...
                await expectRevert(
                    ...
                    'PricyAuction.resultAuction: no open bids'
                );
            });
        ...
        describe('validation', async () => {
            ...
            it('cannot cancel if not an admin', async () => {
                await expectRevert(
                    ...
                    'PricyAuction.cancelAuction: Sender must be item owner'
                );
            });
