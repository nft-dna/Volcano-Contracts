...
contract VolcanoBundleMarketplace is Initializable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    mapping(address => mapping(uint256 => EnumerableSetUpgradeable.Bytes32Set)) bundleIdsPerItem;
    ...
    modifier onlyContract() {
        require(
            addressRegistry.auction() == msg.sender ||
    ...
    function listItem(
        ...
        require(
            owners[bundleID] == address(0) ||
                (owners[bundleID] == msg.sender &&
                    listings[msg.sender][bundleID].price == 0),
            "already listed"
        );
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    IVolcanoTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );

        Listing storage listing = listings[msg.sender][bundleID];
        delete listing.nfts;
        ...
        for (uint256 i; i < _nftAddresses.length; i++) {
            if (_supportsInterface(_nftAddresses[i], INTERFACE_ID_ERC721)) {
                ...
                _check721Owning(_nftAddresses[i], _tokenIds[i], msg.sender);
                require(
    ...
    function cancelListing(string memory _bundleID) external nonReentrant {
        bytes32 bundleID = _getBundleID(_bundleID);
        require(listings[msg.sender][bundleID].price > 0, "not listed");
        _cancelListing(msg.sender, _bundleID);
    }
	...
	*/
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            IVolcanoMarketplace(addressRegistry.marketplace()).validateItemSold(
                listing.nfts[i],
                listing.tokenIds[i],
                owner,
                msg.sender
        ...
        listing.price = 0;
        listings[msg.sender][bundleID] = listing;
        owners[bundleID] = msg.sender;
        delete (offers[bundleID][msg.sender]);
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            IVolcanoMarketplace(addressRegistry.marketplace()).getPrice(_payToken),
            price
        );
        emit OfferCanceled(msg.sender, _bundleID);
    }
    ...
    function createOffer(
        ...
        Offer memory offer = offers[bundleID][msg.sender];
        require(offer.deadline <= _getNow(), "offer exists");

        offers[bundleID][msg.sender] = Offer(_payToken, _price, _deadline);
    ...
    {
        bytes32 bundleID = _getBundleID(_bundleID);
        require(owners[bundleID] == msg.sender, "not owning item");
        ...
        // Transfer NFT to buyer
        Listing memory listing = listings[msg.sender][bundleID];
        for (uint256 i; i < listing.nfts.length; i++) {
            if (_supportsInterface(listing.nfts[i], INTERFACE_ID_ERC721)) {
                IERC721(listing.nfts[i]).safeTransferFrom(
                    msg.sender,
        ...
        delete (listings[msg.sender][bundleID]);
        listing.price = 0;
        ...
        emit ItemSold(
            ...
            IVolcanoMarketplace(addressRegistry.marketplace()).getPrice(address(offer.payToken)),
            offer.price
        );
